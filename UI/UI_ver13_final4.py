...
class ObjectDetection(QThread):
    ...
    def __init__(self, model_type):
        ...
        self.log_timeout = 0 #default: get log as soon as NG is detected
        # When NG is found, log_timeout becomes a predetermined number, like, say, 300
        # Each frame iteration afterwards will reduce the timeout by 1, until log_timeout becomes zero again, which, then stops subtracting
        # While log_timeout > 0, presence of NG boxes will NOT print logs
    ...
    def run(self):
        ...
        while self._run_flag_YOLO:
            ...
            frame,ng_count = self.plot_boxes(results, frame)  #ADDITIONAL CODE: ng_count also returned by box plot function
            if ret:
                self.change_pixmap_signal_YOLO.emit(frame)
                #>> ADDITIONAL CODE: NG box count
                self.update_NG_count.emit(ng_count)
    ...
    def generate_log(self):
        ...
        #Input: tentative (maybe the PyTorch results dict/tree?) 
        pass
...
class MyApp(QWidget):
    ...
    def video_thread_YOLO(self):
        ...
        self.thread_YOLO.update_NG_count.connect(self.updateNGcount)
        # start the thread
    ...
    @pyqtSlot(int)
    def updateNGcount(self, ng_count):
        # Receives a signal from the Object Detection Thread, number of NG boxes
        self.ng_count_label.setText(f"NG: {ng_count}")
